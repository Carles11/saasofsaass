...
export const blocks = pgTable('blocks', {
  ...
  index('blocks_tenant_order_idx').on(t.tenantId, t.order),
])
...
// ============================================
export const tenantTranslations = pgTable('tenant_translations', {
  id:                uuid('id').primaryKey().defaultRandom(),
  tenantId:          uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  entityId:          uuid('entity_id').notNull().references(() => tenantEntities.id, { onDelete: 'cascade' }),
