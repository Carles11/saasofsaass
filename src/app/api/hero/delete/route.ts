...
export async function DELETE(req: NextRequest) {
  ...
  if (!s3Key.startsWith(`${tenant.id}/hero/`)) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

  // Find image in DB
  const image = await db.select().from(heroImages).where(eq(heroImages.s3Key, s3Key)).limit(1);
  ...
  await db.delete(heroImages).where(eq(heroImages.s3Key, s3Key));
